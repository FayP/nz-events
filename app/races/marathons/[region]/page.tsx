...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const allEvents = await getMarathonEvents();
  ...
  const description = `Find ${events.length} upcoming marathon races in ${regionName}, New Zealand for ${year}. Browse 42.2km events with dates, distances, and registration links.`;

  return {
    title: `Marathon Races in ${regionName}, New Zealand`,
    description,
    openGraph: {
      title: `Marathon Races in ${regionName} | GoStride`,
      description,
      type: "website",
    },
  };
...
export default async function RegionMarathonsPage({ params }: PageProps) {
  const resolvedParams = params instanceof Promise ? await params : params;
  const regionSlug = resolvedParams.region;
      ...
      <div className="mx-auto max-w-7xl px-4 pt-8 pb-8">
        ...
        <div className="mb-12">
          ...
          <p className="max-w-3xl text-base leading-relaxed text-muted-foreground">
            ...
            >
              all marathons across New Zealand
            </Link>
            .
          </p>
        </div>
        ...
        {regions.length > 0 && (
          <div className="mb-6 flex flex-wrap items-center gap-2">
            ...
            {regions.map((region) => (
              <Link
                key={region}
                href={`/races/marathons/${regionToSlug(region)}`}
        ...
        <div className="mb-6">
          <p className="text-sm text-muted-foreground">
            {events.length} upcoming marathon
        ...
        {events.length === 0 ? (
          <div className="rounded-xl border border-border bg-card p-12 text-center">
            <p className="text-muted-foreground">
              No upcoming marathons in {regionName} right now. Browse{" "}
              <Link
                href="/races/marathons"
                className="text-foreground underline underline-offset-4 hover:no-underline"
              >
          ...
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              },
              {
                ...
                position: 2,
                name: "Marathons",
                item: `${baseUrl}/races/marathons`,
              },
              {
                "@type": "ListItem",
                position: 3,
                name: regionName,
                item: `${baseUrl}/races/marathons/${regionToSlug(regionName)}`,
              },
            ],
          }),
        }}
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              "@context": "https://schema.org",
              "@type": "ItemList",
              name: `Marathon Races in ${regionName}`,
